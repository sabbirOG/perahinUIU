...
(function () {
	...
	function filterList(term, trimester) {
		...
		items.forEach((el) => {
			...
			const match = (!q || text.includes(q)) && trimesterMatch;
	...
	if (trimesterSelect) trimesterSelect.addEventListener('change', () => filterList(input.value, trimesterSelect.value));
	...
	if (clearBtn) clearBtn.addEventListener('click', () => { input.value=''; filterList('', trimesterSelect ? trimesterSelect.value : 'all'); input.focus(); });
